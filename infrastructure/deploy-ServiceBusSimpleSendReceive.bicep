...
EOF
   ...
   emacs ESC 5 F10
   Begin commands to deploy this file using Azure CLI with PowerShell
   write-output "step 5 create service bus "
   ...
   emacs ESC 8 F10
   Begin commands to deploy this file using Azure CLI with PowerShell
   write-output "step 8 Create Linux  Function App Plan "
   az functionapp plan create --name "$($env:funcPlanName)-linux" --resource-group $env:rg --sku B1 --location $env:loc --is-linux true
   End commands to deploy this file using Azure CLI with PowerShell

   emacs ESC 9 F10
   Begin commands to deploy this file using Azure CLI with PowerShell
   write-output "step 9 Create Function App Windows Plan Skip this because to create a free function app we cannot explicity create a plant"
   ...
   emacs ESC 10 F10
   Begin commands to deploy this file using Azure CLI with PowerShell
   write-output "step 10 show Storage Account for Function App"
   write-output "az storage account show-connection-string --resource-group $env:rg --name $env:storageAccountName --output TSV"
   az storage account show-connection-string --resource-group $env:rg --name $env:storageAccountName --output TSV
   ...
   emacs ESC 11 F10
   Begin commands to deploy this file using Azure CLI with PowerShell
   write-output "step 11 Create Windows Function App"
   ...
   emacs ESC 12 F10
   Begin commands to deploy this file using Azure CLI with PowerShell
   write-output "step 12 Create Linux Function App WARNING: dotnet-isolated will reach EOL on 2024-11-12 and will no longer be supported. https://azure.microsoft.com/en-us/updates/dotnet6support/"
   ...
   emacs ESC 13 F10
   Begin commands to deploy this file using Azure CLI with PowerShell
   write-output "step 13 Configure Linux Function App: Why does this giv an error: ERROR: Runtime  is not supported."
   ...
   emacs ESC 14 F10
   Begin commands to deploy this file using Azure CLI with PowerShell
   write-output "step 14 Delete Function App"
...
Read more about the command in reference docs
   ...
   emacs ESC 17 F10
   Begin commands to deploy this file using Azure CLI with PowerShell
   write-output "step 17 configure windows function app"
   ...
   emacs ESC 18 F10
   Begin commands to deploy this file using Azure CLI with PowerShell
   write-output "step 18 deploy compiled C# code deployment to azure resource. For Linux Func created with azure cli this gives error: ERROR: Runtime  is not supported."
   ...
   emacs ESC 19 F10
   Begin commands to deploy this file using Azure CLI with PowerShell
   write-output "step 19 webapp tail logs"
   ...
   emacs ESC 20 F10
   Begin commands to deploy this file using Azure CLI with PowerShell
   write-output "step 20 get the logs This is not working and I don't now why"
